...
def parse_arguments():
    ...
    parser.add_argument(
        "--theme",
        choices=["matrix", "cyberpunk", "minimal", "nemo"],
        default="nemo",
        help="CLI theme (default: nemo)"
    )
    ...
    parser.add_argument(
        "--new",
        action="store_true",
        help="Start a fresh session (ignore any resume preference)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="SESSION",
        help="Resume a previous session by number or ID"
    )
...
async def main():
    ...
    try:
        # Determine resume session (unless --new flag is set)
        resume_session = None
        if not args.new and args.resume:
            resume_session = args.resume
        
        # Start the CLI
        await cli.start(no_banner=args.no_banner, resume_session_id=resume_session)
