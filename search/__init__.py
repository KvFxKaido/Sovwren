...
implementing the Librarian Pattern:
    ...
    Steward -> Search Gate (consent) -> Sovwren -> Librarian prompt -> Search API
                                                                           |
                                                  JSON: {url, title, snippet}
                                                                           |
    Citations Panel <- Context injection <- Sovwren synthesizes answer citing sources

The local model (Sovwren) never trusts the API's answer directly.
